...
func (e *ExcelImportService) ImportVideoContentFromExcel(filePath string) (successCount int, failCount int, err error) {
	...
	for i := 1; i < len(rows); i++ {
		...
		// 检查是否已存在相同的视频链接
		var existingRecord model.VideoContentAnalysis
...
func (e *ExcelImportService) parseRowToVideoContent(row []string, fieldMap map[string]int) (model.VideoContentAnalysis, error) {
