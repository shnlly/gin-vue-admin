...
var VideoContentAnalysisServiceApp = new(VideoContentAnalysisService)

// CreateVideoContentAnalysis 创建视频内容分析记录
func (vca *VideoContentAnalysisService) CreateVideoContentAnalysis(videoContent *example.VideoContentAnalysis) (err error) {
...
func (vca *VideoContentAnalysisService) UpdateVideoContentAnalysis(videoContent example.VideoContentAnalysis) (err error) {
	err = global.GVA_DB.Model(&example.VideoContentAnalysis{}).Where("id = ?", videoContent.ID).Updates(&videoContent).Error
	return err
}

// GetVideoContentAnalysis 根据ID获取视频内容分析记录
func (vca *VideoContentAnalysisService) GetVideoContentAnalysis(ID string) (videoContent example.VideoContentAnalysis, err error) {
	err = global.GVA_DB.Where("id = ?", ID).First(&videoContent).Error
	return
}

// GetVideoContentAnalysisInfoList 分页获取视频内容分析记录
func (vca *VideoContentAnalysisService) GetVideoContentAnalysisInfoList(info exampleReq.VideoContentAnalysisSearch) (list []example.VideoContentAnalysis, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	// 创建db
	db := global.GVA_DB.Model(&example.VideoContentAnalysis{})
	var videoContents []example.VideoContentAnalysis
...
func (vca *VideoContentAnalysisService) GetVideoContentAnalysisStatistics() (statistics exampleRes.VideoContentAnalysisStatistics, err error) {
	db := global.GVA_DB.Model(&example.VideoContentAnalysis{})
...
func (vca *VideoContentAnalysisService) ImportVideoContentAnalysisFromExcel(filePath string) (successCount int, failCount int, err error) {
	// 先验证Excel文件格式
	if err := ExcelImportServiceApp.ValidateExcelFormat(filePath); err != nil {
		return 0, 0, fmt.Errorf("Excel文件格式验证失败: %v", err)
	}

	// 调用Excel导入服务
	return ExcelImportServiceApp.ImportVideoContentFromExcel(filePath)
}
...
func (vca *VideoContentAnalysisService) BatchUpdateProcessStatus(ids []uint, statusType string, status bool) (err error) {
	...
	err = global.GVA_DB.Model(&example.VideoContentAnalysis{}).Where("id IN ?", ids).Updates(updateData).Error
	return err
}
